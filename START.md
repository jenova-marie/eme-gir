...
- `find_phrase_pattern(pattern, limit)` — same corpus index queried by structural template. Each pattern slot is a literal cf, a POS code (`N`, `V/t`, `RN`, `V*` for any verb), or `*`. Returns ranked attested n-grams. Use when the question is "what shape did scribes write?" rather than "what's near this specific lemma?" — e.g. `["RN","lugal"]`, `["N","du"]`.
