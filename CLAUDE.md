...
| `find_phrase_pattern(pattern, limit)` | Same `collocations.sqlite` index queried by a structural template. Each slot in `pattern` is a literal cf (`'lugal'`), a POS code (`'N'`, `'V/t'`, `'RN'`), a POS family glob (`'V*'`), or `'*'` for any cf. Returns ranked n-grams matching positionally, with each slot annotated by the highest-icount entry's POS + gw (resolves homograph display via `ROW_NUMBER() OVER (PARTITION BY cf ORDER BY icount DESC)`). Implementation ATTACHes `glossary.sqlite` to `collocations.sqlite` for the POS join and uses `EXISTS` subqueries on `entries.cf` to keep the cf1/cf2/cf3/cf4 indexes hot. **Patterns over citation forms only** — no case-marker filtering possible without a separate inflected-collocations build (deferred). Errors on length<2 or length>4 (the index only holds 2/3/4-grams). |
