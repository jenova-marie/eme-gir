...
A workspace for **parsing the Oracc / Eme-gir (electronic Pennsylvania Sumerian Dictionary) corpus and exposing it to LLM agents** for English ↔ Sumerian translation. Oracc is "The Open Richly Annotated Cuneiform Corpus" hosted at `https://oracc.museum.upenn.edu`. The repo is organized into three layers:

### Layer 1 — top-level scripts (build pipeline + entry points)
...
- `build_inflected_collocations.py` — case+sense-aware n-grams (cf, gw, pos, case per slot) using the same suffix peeler as `parse_phrase`. Powers v2/v3 syntax in `find_phrase_pattern`.
...
- `build_cdli_db.py` — downloads the [CDLI](https://cdli.earth) bulk catalogue (`cdli_cat.csv`, ~147 MB) from the cdli-gh GitHub mirror via media.githubusercontent.com (LFS-direct, no `git lfs` install needed), parses 353K artifact rows × 64 columns into `data/cdli.sqlite` (~157 MB) keeping a curated subset of ~25 columns we actually use. Powers `lookup_artifact` + `find_artifacts` AND the photo/lineart URL enrichment that gets splatted onto every cited line by `see_examples` / `find_verb_form`. CDLI catalogue text is freely reusable with citation per CDLI's terms; imagery on cdli.earth is non-commercial only. We host no images — all image URLs point straight to cdli.earth so image-licensing remains CDLI's domain.
...
- `mcp_server.py` — **Legacy all-in-one** MCP server. Registers all 21 tools and 2 resources in a single FastMCP instance. Kept for backwards compatibility with pre-Phase-5 deployments; new deployments should prefer the per-domain `servers/<name>` modules. The 21 tools are: thirteen ePSD2 dictionary + corpus tools (`translate_english`, `translate_sumerian`, `parse_phrase`, `lookup_entry`, `see_examples`, `find_compound`, `find_collocations`, `find_phrase_pattern`, `get_inflections`, `analyze_form`, `find_verb_form`, `lookup_sign`, `cuneify`) + four ETCSL literary tools (`etcsl_search_english`, `etcsl_lines_with_lemma`, `etcsl_search_sumerian`, `etcsl_lookup_text` — bilingual because ETCSL ships English translations) + two CDLI tools (`lookup_artifact`, `find_artifacts`) + two bootstrap wrappers (`start_here`, `get_grammar_reference`). `parse_phrase` is the case-aware grammatical pre-annotator: peels suffixes, classifies tokens by phrase role, emits a compact bracket skeleton. `start_here()` leads its docstring with "⭐ CALL THIS FIRST" so a tools-list scan naturally surfaces it as the entry point.

### Layer 2 — `servers/` (per-domain MCP entry points, Phase 5)

Each entry point is a thin (~30-40 line) script registering only its domain's tools. Run via `python -m servers.<name>` for stdio (Claude Code), or `python -m servers.<name> --transport http` for HTTP.

- `servers/epsd2/__main__.py` — 11 ePSD2 dictionary + corpus tools; HTTP port 5052; requires `data/glossary.sqlite` + `data/text_index.sqlite`.
- `servers/etcsl/__main__.py` — 4 ETCSL literary tools (bilingual; ETCSL is NOT under any Creative Commons license — traditional academic copyright with required Oxford citation, see LICENSE-DATA.md); HTTP port 5053; requires `data/etcsl.sqlite`.
- `servers/cdli/__main__.py` — 2 CDLI artifact catalogue tools; HTTP port 5054; requires `data/cdli.sqlite`.
- `servers/ogsl/__main__.py` — 2 cuneiform sign rendering tools (`cuneify`, `lookup_sign`); HTTP port 5055; requires `corpus/ogsl.zip`. Reusable for Akkadian/Hittite/Hurrian/Elamite — cuneiform isn't Sumerian-specific.
- `servers/translator/__main__.py` — bootstrap surface: registers the two MCP resources (`oracc://prompt/agent`, `oracc://grammar/sumerian`) and the two tool wrappers (`start_here`, `get_grammar_reference`). **No data tools** — agents are expected to compose this with the four data servers.

### Layer 3 — `eme_gir/` (shared Python package, Phases 1-4)

Imported by every entry point + build script. Internal imports use relative form (`from .paths import ...`); external imports use absolute (`from eme_gir.paths import ...`).

- `eme_gir/paths.py` — single source of truth for project file locations (DATA_DIR, LOG_DIR, GLOSSARY_DB, etc.). Every other module imports its canonical paths from here. `ROOT = Path(__file__).resolve().parent.parent` so paths resolve to the repo root regardless of which entry point started the process.
- `eme_gir/log.py` — shared logging configuration + the `log_call` decorator. `init_logging(server_name)` configures the root logger (stderr + rotating-file handler) and writes to `log/<server_name>.log` so parallel-running servers don't clobber each other. The `log_call` decorator (formerly `_log_call` in mcp_server.py) wraps each tool function for entry/exit/timing logs + umami analytics emission.
- `eme_gir/server.py` — shared server boilerplate: `make_server(name, instructions)` factory that wires up Auth0 OAuth (via `EME_GIR_REQUIRE_AUTH`) + DNS-rebinding transport security (via `EME_GIR_ALLOWED_HOSTS`/`_ORIGINS`); `run_server(mcp, log, default_port, required_dbs=...)` for the argparse + startup banner + transport selection loop; `READ_ONLY_ANNOTATIONS` constant.
- `eme_gir/cuneify.py` — converts Oracc transliteration (`{d}lugal`, `lu₂-gal`, `peš₁₀-peš₁₀-e\l`) into Unicode cuneiform glyphs (𒀭𒈗, 𒇽𒃲, 𒆠𒀀𒆠𒀀𒂊 — `peš₁₀` is the compound sign `|KI.A|`, so each instance renders as two glyphs `𒆠𒀀`). Loads the OGSL sign list from `corpus/ogsl.zip` once at import; exposed as a Jinja `cuneify` filter by `app.py`. ~93% of glossary spellings render with full glyph coverage.
- `eme_gir/text_resolver.py` — resolves glossary `word_ref` strings (e.g. `epsd2/admin/ur3:P113959.10.3`) into the actual line of Sumerian text by lazy-loading the right `corpusjson/{P-id}.json` from inside its zip. LRU-cached per-text.
...
### Layer 4 — data + prompts (the corpus itself)

- `corpus/` — 208 zip files (~3.1 GB), one per Oracc project; this is the bulk dataset
- `data/` — generated SQLite indexes live here (auto-created on first run via `eme_gir/paths.py`):
...
- `build_collocations.py` — walks every corpusjson text in `corpus/`, extracts 2/3/4-grams of citation forms within each line, stores counts in `collocations.sqlite`. ~138K texts → ~178K collocations kept (default `--min-count 3`).
- `build_text_index.py` — scans every project zip in `corpus/` for `*/corpusjson/P*.json` files and builds `text_index.sqlite`, a `(project, text_id) → (zip_path, member_path)` map. Takes ~2 s for the full 208 zips.
- `log/` — generated server logs live here. Each MCP server writes to its own file: `log/mcp_server.log` (legacy), `log/eme-gir-epsd2.log`, `log/eme-gir-etcsl.log`, `log/eme-gir-cdli.log`, `log/eme-gir-ogsl.log`, `log/eme-gir-translator.log`. All written by the `log_call` decorator (rotating, 5 MB × 3 backups). Tail with `tail -F log/eme-gir-*.log` to watch agent activity in real time across all running servers.
- `prompt/` — checked-in prompt + reference material for LLM agents:
  - `lessons/JAGERSMA_GRAMMAR.md` — ~40 KB comprehensive Sumerian grammar cheat sheet distilled from Jagersma 2010 (Bram Jagersma, *A Descriptive Grammar of Sumerian*, PhD dissertation, Universiteit Leiden, 2010, 776 pp). Every grammatical claim carries an inline Jagersma §-citation for verification. Covers phonology, the twelve enclitic cases (with surface-ambiguity tables), the nine-slot finite-verb template, perfective/imperfective inflection patterns, all preformatives, dimensional prefixes, non-finite forms, copular clauses, and nominalization-based subordination. Default period for translation when unspecified is **ED (Early Dynastic, ~2900-2350 BCE)** — Jagersma's primary descriptive ground (Old Sumerian = ED IIIa-IIIb). Companion audit-trail files: `lessons/JAGERSMA_CONVENTIONS.md` (Jagersma → Oracc notation map; period labels; citation format) and `lessons/JAGERSMA_NOTES.md` (per-chapter structured digest of the 31-chapter source, ~17K words, every claim cited).
  - `lessons/MEADOW_GRAMMAR.md` — ~48 KB temple-register grammar companion. Distilled from Meadow's Sumerian 101 classroom-e₂-nun-na lecture series + Entu Siri Nin's commentary. Prayer-ready pedagogy (the PNC mnemonic, the "pesky -a" three-tip heuristic), the Emesal liturgical register, worked temple-composition examples. Cite as `(Meadow §101-N)` or `(Siri Nin)`. The MCP resource `oracc://grammar/sumerian` and the `get_grammar_reference()` tool BOTH return Jagersma + Meadow concatenated (~80 KB combined) with a separator banner; conflict resolution per §18 of MEADOW_GRAMMAR.md — academic file normative for reading attested texts; temple file normative for composing new in-temple Sumerian.
  - `prompt/AGENT_PROMPT.md` — drop-in system prompt that teaches an LLM agent how to use the MCP servers' tools end-to-end (11-step English→Sumerian workflow + reverse direction + worked example). Distinct from JAGERSMA_GRAMMAR.md: the prompt teaches HOW to use the tools, the grammar teaches WHAT Sumerian is. Shipped both as a file AND as the MCP resource `oracc://prompt/agent` (served by the Translator server). Enforces CDLI link-rendering as Markdown hyperlinks for every tool result that carries `cdli_url` / `photo_url` / `lineart_url`.
- `README.md` — top-level project README aimed at humans cloning the repo.
- `.mcp.json` — project-scoped MCP server config; Claude Code auto-detects this when launched in the repo. **Six entries** post-Phase-5: `eme-gir` (legacy all-in-one → `mcp_server.py`), `eme-gir-epsd2` (→ `python -m servers.epsd2`), and three siblings for etcsl/cdli/signs, plus `eme-gir-translator` (bootstrap-only). **Uses absolute paths** because Claude Code spawns MCP server processes without sourcing the user's shell init, so a bare `python3` resolves to `/usr/bin/python3` (no `mcp` package). Per-domain entries also set `cwd` so `python -m servers.X` resolves the `servers/` package.
...
**Data licenses (see [`LICENSE-DATA.md`](LICENSE-DATA.md) for the full statement):**
- **Oracc / ePSD2 / OGSL** — **CC BY-SA 3.0 Unported** (per Oracc's project-wide license at `oracc.museum.upenn.edu/doc/about/licensing`). Attribution required; ShareAlike propagates to substantial reuses.
...
### OAuth 2.1 / Auth0 authorization (`eme_gir/auth0_verifier.py`)
...
### Transport security / DNS-rebinding allowlist (`eme_gir.server._build_transport_security_kwargs`)
...
## Cuneiform rendering (`eme_gir/cuneify.py`)
...
## Attestation rendering (`eme_gir/text_resolver.py`)
